...
from transcendence_django.dict_keys import ONLINE_KEY, PLAYING_KEY, OFFLINE_KEY, FRIENDS_REQUESTS_KEY
...
def accept_friendship(request):
    try:
        ...
        if user.accept_friendship_request(friend) is not None:
            ...
            return JsonResponse({
                "error": "Friend request does not exist."}, status=status.HTTP_400_BAD_REQUEST
            )
    except CustomUser.DoesNotExist:
        return JsonResponse(
            {"error": "User does not exist."}, status=HTTPStatus.NOT_FOUND
...
def get_friends_info(request):
    ...
    try:
        ...
        return JsonResponse({
            ...
            OFFLINE_KEY: offline_friends}, status=status.HTTP_200_OK)
    except CustomUser.DoesNotExist:
        return JsonResponse(
            {"error": "User does not exist."}, status=HTTPStatus.NOT_FOUND
        )
